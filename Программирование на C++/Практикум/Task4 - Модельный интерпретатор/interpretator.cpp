...
class Lex
{
	...
	string s_lex;
public:	
	Lex ( type_of_lex t = LEX_NULL, int v = 0, string s = ""): t_lex (t), v_lex (v), s_lex (s) {}

	type_of_lex  get_type () { return t_lex; }
	int get_value () { return v_lex; }
	string get_str () { return string(s_lex); }
	friend ostream & operator<< (ostream &s, Lex l)
	{
		s << '(' << l.v_lex << "); ==> " << lex_out[l.t_lex];
		if (l.s_lex.size () != 0) s << " ==> " << l.s_lex;
		return s;
	}
};
...
Lex Scanner::get_lex () 
{
	...
	do
	{
		switch(CS)
			...
			case STR:
				if (feof(fp)) throw "LEX: EOF!!! The string doesn't end";
				if (c == '"')
				{
					gc();
...
void Parser :: P ()
{
	if (c_type != LEX_PROGRAM) throw "PARSER: Expected program"; gl();
	if (c_type != LEX_BEGIN) throw "PARSER: Expected {"; gl();
	
	while (c_type == LEX_INT || c_type == LEX_BOOL || c_type == LEX_STRING)
	{
		D1();
	  	if (c_type != LEX_SEMICOLON) throw "PARSER: Expected ;"; gl();
	}

	B();

  	if (c_type != LEX_FIN) throw "PARSER: Expected }";
}
...
void Parser :: D1 ()
{
	...
	{
		gl ();
		if (c_type != LEX_ID) throw "PARSER: Expected Identificator";
		if (TID[c_val].get_declare() ) throw "PARSER: Twice";
		else
		{
		...
		st_lex.push (TID [c_val].get_type () ); // ~ check_id ();
...
void Parser::S ()
{
	...
	/* IF () {} ELSE {} */
  	if (c_type == LEX_IF)
  	{
		...
		if (c_type != LEX_LPAREN) throw "PARSER: Expected ("; gl ();

    		E();
    		eq_bool();

		if (c_type != LEX_RPAREN) throw "PARSER: Expected )"; gl ();		
		...
		if (c_type != LEX_END) throw "PARSER: Expected }"; gl ();		
		if (c_type == LEX_ELSE)
		{
			gl();
			if (c_type != LEX_BEGIN) throw "PARSER: Expected {"; gl ();
		...
		else prog.put_lex (Lex(POLIZ_LABEL, prog.get_free()), pl3);
  	}
	/* DO {} WHILE (); */ //!!! POLIZ
  	else if (c_type == LEX_DO)
	{
		bool previous_flag = am_i_in_cycle; am_i_in_cycle = true;
		int prev_break_num = break_counter; break_counter = 0;
		...
		if (c_type != LEX_END) throw "PARSER: Expected }"; gl ();
    		if (c_type != LEX_WHILE) throw "PARSER: Expected while"; gl ();
		if (c_type != LEX_LPAREN) throw "PARSER: Expected ("; gl ();
		...
		{
			prog.put_lex( Lex (POLIZ_LABEL, prog.get_free() ), j = st_break.pop() );
		}
	...
	/* READ */
  	else if (c_type == LEX_READ)
  	{
    		...
    		if (c_type != LEX_LPAREN) throw "PARSER: Expected (";
	...
	/* ASSIGN */
  	else if ( c_type == LEX_ID )
  	{
		...
		if (c_type != LEX_ASSIGN) throw "PARSER: Expected ="; gl ();
	...
	else if ( c_type == LEX_BREAK )
	{
  	...
  	else if ( c_type != LEX_FIN && c_type != LEX_BEGIN )
	{
		cerr << "S -> no way\n";
...
void Parser::E5 () 
{
	if ( c_type == LEX_ID ) 
  	{
    		check_id();
    		prog.put_lex (Lex (LEX_ID, c_val, c_str)); 
    		gl();
  	}
	...
	int i;
  	while ( !st_int.is_empty()) 
  	{
    		...
    		if ( TID[i].get_declare() ) 
      			throw "PARSER: Double declaration";
	    	else 
	    	{
	      		TID[i].put_declare();
	      		TID[i].put_type(type);
...
{
	if ( st_lex.pop() != LEX_BOOL )
    		throw "PARSER: Expression is not bool";
}
...
{
	enum state_now {INT, STR};
	int flag = INT;
	Stack < int, 100 >    args;
	Stack < string, 100 > str;
	...
	while ( index < size )
	{
		pc_el = prog [ index ];
			...
			case LEX_STRING:
				...
				if (TID[i].get_str().size() != 0) flag = STR;
				else flag = INT;
				
				if ( !TID[i].get_assign () ) throw "POLIZ: indefinite identifier";
				...
				break;
	      		...
	      		case LEX_WRITE:
				if (flag == INT) cout << args.pop () << " ";
				if (flag == STR) cout << str. pop () << endl;;
				break;
	      		...
	      		case LEX_READ:
			{
				...
				break;
		      	...
		      	case LEX_PLUS:
				if (flag == STR) str.push ( str.pop() + str.pop() );
			...
			case LEX_EQ:
				if (flag == STR) args.push ( str.pop() == str.pop() );
				else args.push ( args.pop() == args.pop() );
				break;
...
int main (int argc, char* argv[])
{
	...
	{
		cout << "\nThe analization has finished with an ERROR!\n\n";
    		cout << "LEX: unexpected symbol " << c << endl << endl;
    		return 1;
 	}
  	catch (Lex l)
  	{
